...
def temporal_shift_image_mode(x, channel_shift_fraction=0.125, alpha=0.3):
    ...
    shifted_x = jnp.concatenate([shifted_backward, no_shift, shifted_forward], axis=3)
    return shifted_x
...
def temporal_shift_gpu(
    ...
    shifted_x = jnp.concatenate([shifted_backward, no_shift, shifted_forward], axis=4)
...
def temporal_shift_tpu(
    ...
    x_backward_padding = jax.lax.pad(
        x, padding_value=jnp.bfloat16(0.0), padding_config=shifted_backward_padding
    )
    # The following shift gets to [x3^+1, 0, 0] (where +1 means from the future).
    shifted_backward = jax.lax.slice(
        x_backward_padding,
        (batch_size, 0, 0, n_channels - n_shift),
        (orig_shp[0] + batch_size, orig_shp[1], orig_shp[2], 2 * n_channels - n_shift),
    )
    ...
    x_forward_padding = jax.lax.pad(
        x, padding_value=jnp.bfloat16(0.0), padding_config=shifted_forward_padding
    )
