...
class ResUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # [#src, #inplanes, H/2, W/2]. x: [#src, 3, H, W]

        x1 = self.layer1(x)  # [#src, #inplanes, H/4, W/4]
        x2 = self.layer2(x1)  # [#src, 128, H/8, W/8]
        x3 = self.layer3(x2)  # [#src, 256, H/16, W/16]

        x = self.upconv3(x3)  # [#src, 128, H/8, W/8]
        x = self.skipconnect(x2, x)  # [#src, 2x128, H/8, W/8]
        x = self.iconv3(x)  # [#src, 128, H/8, W/8]

        x = self.upconv2(x)  # [#src, #inplanes, H/4, W/4]
        x = self.skipconnect(x1, x)  # [#src, 2x#inplanes, H/4, W/4]
        x = self.iconv2(x)  # [#src, 32, H/4, W/4]

        x_out = self.out_conv(x)  # [#src, 32, H/4, W/4]
        if self.single_net:
            x_coarse = x_out
            x_fine = x_out
        else:
