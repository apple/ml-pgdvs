# Copy from https://github.com/zhengqili/Neural-Scene-Flow-Fields/blob/d4001759a39b056c95d8bc22da34b10b4fb85afb/nsff_exp/models/networks_basic.py
...
from .. import nsff_lpips as util
...
def spatial_average(in_tens, mask=None, keepdim=True):
    if mask is None:
        ...
        mask_resized = torch.nn.functional.interpolate(
            mask[:, 0:1, :, :], size=[in_tens.size(2), in_tens.size(3)]
        )
...
class PNetLin(nn.Module):
    ...
    def forward(self, in0, in1, mask=None, retPerLayer=False):
        ...
        if self.lpips:
            if self.spatial:
                res = [
                    upsample(self.lins[kk].model(diffs[kk]), out_HW=in0.shape[2:])
                    for kk in range(self.L)
                ]
            else:
                res = [
                    spatial_average(self.lins[kk].model(diffs[kk]), mask, keepdim=True)
                    for kk in range(self.L)
                ]
        else:
            if self.spatial:
                ...
                res = [
                    spatial_average(
                        diffs[kk].sum(dim=1, keepdim=True), mask, keepdim=True
                    )
                    for kk in range(self.L)
                ]
